...
def load_documents():
    faq_data = []
    faq_file = os.path.join(BASE_PATH, "faq.txt")
    ...
    policies = []
    policies_file = os.path.join(BASE_PATH, "company policies.txt")
...
def build_index(docs):
    embedder = load_model()
    doc_embeddings = embedder.encode(docs, show_progress_bar=True).astype("float32")
    index = faiss.IndexFlatL2(doc_embeddings.shape[1])
    index.add(doc_embeddings)
    return embedder, index

embedder, index = build_index(documents)
